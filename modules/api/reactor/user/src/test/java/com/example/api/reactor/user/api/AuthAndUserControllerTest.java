...
import com.example.api.reactor.user.api.handlers.ArgumentVaildExceptionHandler;
import com.example.api.reactor.user.api.handlers.CustomExceptionHandler;
...
public class AuthAndUserControllerTest {
    ...
    void test_fetchUser_invaild_pathParam() throws Exception {
        ...
        .expectBody()
            .jsonPath("$.code").isEqualTo("400 BAD_REQUEST")
    ...
    void test_fetchUser_by_account_notFound() throws Exception {
        when(userManage.getUserByAccount(testDto.getAccount())).thenThrow(new DataNotFoundException(testDto.getAccount()));
        
        webTestClient.get().uri(uriBuilder -> 
            uriBuilder
            .path("/v1/users/account/{account}")
            .build(testDto.getAccount())
        ).exchange()
    ...
    void test_fetchUser_by_account() throws Exception {
        when(userManage.getUserByAccount(testDto.getAccount())).thenReturn(Mono.just(testDto));
        ...
        .expectStatus().isOk()
        .expectBody()
            .jsonPath("$.userId").isEqualTo(testDto.getUserId())
            .jsonPath("$.account").isEqualTo(testDto.getAccount());
    }
    ...
    void test_signUp_payload_invaild(String account, String password, String userName, LocalDateTime birthDay) throws Exception {
        ...
        webTestClient.post().uri(builder -> builder.path("/v1/auth/sign-up").build())
    ...
    void test_signUp() throws Exception {
        ...
        when(authManage.signUp(any())).thenReturn(Mono.just(testDto.getUserId()));
    ...
    void test_signIn_payload_invaild(String account, String password) throws Exception {
        ...
        webTestClient.post().uri(builder -> builder.path("/v1/auth/sign-in").build())
            .contentType(MediaType.APPLICATION_JSON)
            .bodyValue(requestBody.toString())
            .exchange()
            .expectStatus().isBadRequest();
    ...
    void test_signIn() throws Exception {
        ...
        when(authManage.signIn(any())).thenReturn(Mono.just(testDto.getUserId()));
